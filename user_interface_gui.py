...
class interface :
    ...
    def str_hole(self):
        ...
        hole_start_address= float(self.hole_start_entry.get()) #hole start address
        hole_size= float(self.hole_size_entry.get())      #hole size
        self.mm.addHole(hole_start_address,hole_size)
        self.hole_frame.pack_forget()
        self.hole_frame.destroy()

    def run(self):
        self.mm.setSize(float(self.mem_entry.get()))    #size of memory
        self.mm.divideMem()
        self.f.pack_forget()
        self.f.destroy()
        self.f2.pack()
